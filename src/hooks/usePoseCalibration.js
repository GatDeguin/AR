...
export function usePoseCalibration(videoRef, scaleReal = 647) {
  ...
  const runCalibration = useCallback(async () => {
    ...
    try {
      ...
      const nut = {
        x: (frets[0].x1 + frets[0].x2) / 2,
        y: (frets[0].y1 + frets[0].y2) / 2,
      };
      ...
      const { pose, success } = await calibratePose(
        { nut, bridge, fret5, fret12 },
        scaleReal,
        videoRef.current.videoWidth,
        videoRef.current.videoHeight,
      );
      if (!success) throw new Error("solvePnP falló");
      setPoseMatrix4(pose.matrix4);
      setCalibrated(true);
      setError(null);
    } catch (e) {
      console.warn("Calibración fallida", e);
      setError(e.message);
      setCalibrated(false);
    }
  }, [videoRef, scaleReal]);
